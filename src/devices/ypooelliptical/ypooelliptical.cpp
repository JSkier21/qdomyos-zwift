            ...
            if (LIFE_FITNESS_ELLIPTICAL) {
                ...
                if (totalDistanceMeters != lastTotalDistance) {
                    if (lastTotalDistance > 0 && totalDistanceMeters > lastTotalDistance) {
                        ...
                        // Require a sane sampling interval: a short/jittery gap turns the 1m distance
                        // quantum into a huge apparent speed (a 3m step over 200ms reads as 54 km/h).
                        if (elapsedMs >= 500) {
                            double candidate = (((double)(totalDistanceMeters - lastTotalDistance)) / 1000.0) /
                                               (((double)elapsedMs) / 3600000.0);
                            if (candidate < 30.0) { // sanity check: reject spikes, no elliptical does 30 km/h
